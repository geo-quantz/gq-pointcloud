...
### macOS: 「"gqfilter" is damaged and can't be opened」と表示される

GitHub Release からダウンロードした zip には macOS の quarantine 属性が付きます。
展開後、**実行前に**以下を実行してください（`_internal/` 内の dylib にも属性が付いているため `-cr` が必要です）。
...
詳細は [README_macos.md](README_macos.md#gatekeeper-and-security) を参照してください。
